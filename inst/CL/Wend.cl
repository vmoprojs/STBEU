...
double wen_time(double *par, double h,double u)
{
    ...
    double sep=par[5];
    double smooth=par[6];
    //Rprintf("R_power_s: %f R_power: %f R_power_t: %f scale_s: %f scale_t: %f sep: %f smooth: %f \n",R_power_s,R_power,R_power_t,scale_s,scale_t,sep,smooth);
...
double deri_scale_s_wen_time(double *par, double h,double u)
{
    ...
    double delta=sqrt(EPS)*scale_s;
    
    double par1[7];
    //par1 = (double *) Calloc(7, double); /* idem */
    //par1 = (double *) calloc(7, sizeof(double));
    // define vector par1
    par1[0]=R_power;
    par1[1]=2;
    par1[2]=R_power_t;
...
double deri_R_power_t_wen_time(double *par, double h,double u)
{
    ...
    double delta=sqrt(EPS)*R_power_t;
...
void GradCorrFct(double rho, int flag0, int flag1, int flag2, int flag3, int flag4,int flag5,int flag6,double *grad, double h, double u,double par0,double par1,double par2,double par3,double par4,double par5,double par6)
{
    ...
    if(flag0==1){//power2_s parameter
        grad[i]=deri_R_power_wen_time(parodia,h,u);i++;}
    if(flag1==1){//power_s parameter
        grad[i]=0;i++;}
    if(flag2==1){//power2_t parameter
        grad[i]=deri_R_power_t_wen_time(parodia, h,u);i++;}
    if(flag3==1){//scale_s parameter
        grad[i]=deri_scale_s_wen_time(parodia, h,u);i++;}
    if(flag4==1){//scale_t parameter
        grad[i]=deri_scale_t_wen_time(parodia, h,u);i++;}
    if(flag5==1){//sep parameter
        grad[i]=deri_sep_wen_time(parodia, h,u);i++;}
    //smooth parameter
    if(flag6==1)
        grad[i]=deri_smooth_wen_time(parodia, h,u);
    return;
}
/********************************************************************/
/************** correlation  models **********************************/
