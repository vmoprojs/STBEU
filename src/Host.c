...
int DevOpenCL()
{
    ...
    for (i = 0; i < platformCount; i++) {
        ...
        for (j = 0; j < deviceCount; j++) {
            ...
            value = (char*) malloc(sizeof(valueSize));
            ...
            //RRprintf("%d.%d\tCL_DEVICE_MAX_CONSTANT_ARGS\tMax kernel args:\t%zu\n",j, 15,p_size);
            Rprintf("-------------------------------------------------------------\n");
...
void create_binary_kernel(int *dev, char **fname)
{
    ...
    //if(err!=CL_SUCCESS){RRprintf("Failed to get CL_PROGRAM_BINARY_SIZES\n");}
